    ...
    def process_options():
            ...
            for parameters in parameter_list:
                ...
                def math_routine():
                    ...
                    if (math_type.get() == 'Arithmetic') | (math_type.get() == 'Compare'):
                        ...
                        if math_operation.get() == '(im)^n':
                            math_constant = Scale(math_routine_frame, label='Constant Operator', from_=1, to=4, resolution=1, orient=HORIZONTAL, length=300)
                            math_constant.place(x=20, y=90+len(operations)*30)
        ...
        def preview_result():
            ...
            if process_select.get() == "Gabor":
                ...
                if (math_type == 'Arithmetic') | (math_type == 'Compare'):
                    if constant.get() == True:
                        operand2 = math_constant.get()
                    
                    else:
                        operand2 = images[image_names.index(image_source2.get())]
