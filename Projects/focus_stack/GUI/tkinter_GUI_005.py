...
def proc_dev_interface():
    ...
    def process_options():
        ...
        process_ops_dict =   {   "Gabor": [
                                            ...
                                            ['lambda', ' - wavelength', 0.05, 5, .05, 'scale'], 
                                ...
                                "Math": [   ['Arithmetic', 'Compare', 'Logs_and_Exponents'],
                                            ...
                                            {
                                            'Arithmetic': ['Add', 'Subtract', 'Multiply', 'Divide', 'radio'],
                                            'Compare': ['Max', 'Min', 'radio'],
                                            'Logs_and_Exponents': ['(im)^n', 'e^(im)]', 'log(im)', 'radio']
                                            }
                                ]
        ...
        if process_select.get() == "Math":
                ...
                math_type = StringVar()
                math_type.set('None')
                for i in range(len(process_ops_dict["Math"][0])):
                    Radiobutton( process_ops_frame, text=process_ops_dict["Math"][0][i], variable=math_type, command=math_routine, value=process_ops_dict["Math"][0][i]).place(relx=.1, rely = (.1 + i * .1))
