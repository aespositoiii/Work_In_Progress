    ...
    def reg_comb(images, order, trans_on, mask, output_filename, warp_mode=cv.MOTION_EUCLIDEAN, number_of_iterations=1000, termination_eps=1e-3):
        ...
        for i in range(len(order)):
            ...
            if i != 0:
                ...
                t6=time.time()
                print('getwarp : ',t5-t4, '   warp : ', t6-t5)
                '''
                cv.imshow('color',  images[order[i]])
                cv.waitKey(1)
                cv.imshow('mask',  mask[order[i]])
                cv.waitKey(1)
                '''
            print('Register Image ', i, ' of ', len(order)-1, '   t_it : ', time.time() - t1, 's    t_total : ', time.time() - t0, 's')

        mask_select = np.zeros(mask.shape, dtype='uint8')
        mask_select[(mask == np.max(mask, axis=0)) & (mask > 0)] = 1
        mask_select_sum = np.sum(mask_select, axis=0).astype('uint8')
        
        mask_select_sum_vis = np.zeros(mask_select_sum.shape, 'uint8')
        mask_select_sum_vis = mask_select_sum * (255//np.max(mask_select_sum))

        cv.imshow('mask_select_sum', mask_select_sum_vis)

        #mask_select_vis = np.zeros(mask_select.shape, dtype='uint8')

        #for i in range(len(mask_select)):


        '''
        for i in range(mask_select.shape[0]):
            kernel_figure = plt.figure('Mask')
            plt.clf()
            plt.title('Mask_Select')
            plt.imshow(mask[i])
            plt.draw()
            plt.show(block=False)
            cv.imshow('masked', (images[i,:,:,0] * mask[i]).astype('uint8'))
            cv.waitKey(1)
        '''

        comb = np.zeros(images.shape[1:], dtype='uint8')
        images_masked = np.zeros(images.shape, dtype='uint8')
        masked  = np.ones(mask_select_sum.shape, dtype='uint8')
        masked[mask_select_sum>0] = 0

        for i in range(images.shape[3]):
            images_masked[:,:,:,i] = (images[:,:,:,i] * mask_select) // mask_select_sum
            comb[:,:,i] += np.sum(images_masked[:,:,:,i], axis=0)

        dil_it = max(mask_select_sum.shape)//20

        mask_dilate_kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (5,5))
        k = 0
        while masked.max() == 1:
            t2 = time.time()
            for j in range(mask_select.shape[0]):
                t3 = time.time()
                mask_select[j,:,:] = cv.dilate(mask_select[j,:,:], mask_dilate_kernel, iterations = dil_it)
                print('dilate time : ', time.time() - t3)
            print('dilate time : ', time.time() - t2)
            mask_select = mask_select * masked
            mask_select_sum = np.sum(mask_select, axis=0).astype('uint8')
            for i in range(images.shape[3]):
                #images_masked[:,:,:,i][mask_select > 0] = images[:,:,:,i][mask_select > 0] * mask_select[mask_select > 0]
                images_masked[:,:,:,i] = images[:,:,:,i] * mask_select
                images_masked[:,:,:,i] = images_masked[:,:,:,i] // mask_select_sum
                comb[:,:,i] += np.sum(images_masked[:,:,:,i], axis=0).astype('uint8')
            masked[mask_select_sum>0] = 0
            k += 1
            cv.imshow('masked', masked*255)
            cv.imshow('mask_select', mask_select_sum*40)
            cv.imshow('comb', comb)
            cv.waitKey(1)
            print('iter : ', k,'time tot : ', time.time()-t0, ' time it : ', time.time()-t2)
    ...
    def batch_process(process_summary_filename, image_filenames, process_summary_dict,im_save, batch_stack):
        ...
        for i in image_filenames:
            ...
            for j in process_summary_dict:
                ...
                elif process == 'Norm_Shift':
                    result = cv.normalize(images[j['src_im_ind']], result, alpha=1, beta=(images[j['src_im_ind']].max() - images[j['src_im_ind']].min()+1), norm_type=cv.NORM_MINMAX, dtype=cv.CV_8U)
            ...
            if batch_stack == 'stack':
                try:        
                    ...
                    print('append', image_filenames.index(i), color.shape, mask.shape)
                except:
                    color = np.zeros((len(image_filenames), base_color.shape[0], base_color.shape[1], base_color.shape[2]), dtype='uint8')
                    color[image_filenames.index(i),:,:,:] = base_color
                    mask = np.zeros((len(image_filenames), result.shape[0], result.shape[1]), dtype='uint8')
                    mask[image_filenames.index(i),:,:] = result
                    print('append', image_filenames.index(i), color.shape, mask.shape)

                '''
                cv.imshow('color',  color[image_filenames.index(i),:,:,:])
                cv.waitKey(1)
                cv.imshow('mask',  mask[image_filenames.index(i),:,:])
                cv.waitKey(1000)
                '''
