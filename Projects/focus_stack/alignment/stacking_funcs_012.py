...
def image_sort(images, filenames, file_nums, histograms, n_comps, color_channels, hist_thresh):
    ...
    for i in range(image_count):
        ...
        for j in range(image_count):
            ...
            print(i, '  ', file_nums[i], '  ', j, '  ',image_corr_sort[i,j], '  ', file_nums[image_corr_sort[i,j]])
    
    start_image = np.argmax(np.sum(np.sort(image_corr, axis=0)[...,(image_count-6):], axis=0))

    order = [start_image]
    image_list = list(range(image_count)).remove(start_image)

    while len(image_list)>0:
        ref_array = np.ravel(image_corr_sort[order.reverse(),...])
        for i in range(ref_array.shape[0]-1, -1, -1):
            if ref_array[i] not in order:
                order = order.append(ref_array[i])
    ...
    return image_corr_sort, start_image
