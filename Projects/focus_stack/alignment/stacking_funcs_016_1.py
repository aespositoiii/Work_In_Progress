...
def laplace_threshold(src, thresh, norm_blur):
    ...
    src = cv.GaussianBlur(src, (21, 21), 0)
    # [reduce_noise]
...
def mask_blur(img, thresh=15, norm_blur=11, n_iter=50, normalize=True):
...
def reg_comb(images, order, thresh=15, norm_blur=11, n_iter=50, exp=2, warp_mode=cv.MOTION_EUCLIDEAN, number_of_iterations=1000, termination_eps=1e-3):
    ...
    comb_mask = np.zeros(comb_grad.shape, dtype='float32')
    img_mask_warped = np.copy(comb_mask)  
    ...
    for i in order[1:]:
        ...
        comb_mask[ (comb_grad_float > 0) | (img_grad_warped_float > 0)] = comb_grad_float[ (comb_grad_float > 0) | (img_grad_warped_float > 0)] / (comb_grad_float[ (comb_grad_float > 0) | (img_grad_warped_float > 0)]+img_grad_warped_float[ (comb_grad_float > 0) | (img_grad_warped_float > 0)])
        img_mask_warped[ (comb_grad_float > 0) | (img_grad_warped_float > 0)] = img_grad_warped_float[ (comb_grad_float > 0) | (img_grad_warped_float > 0)] / ( comb_grad_float[ (comb_grad_float > 0) | (img_grad_warped_float > 0)]+img_grad_warped_float[ (comb_grad_float > 0) | (img_grad_warped_float > 0)])
