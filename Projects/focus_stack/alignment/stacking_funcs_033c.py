...
def import_describe(directory, ext, hist_min=0, hist_max=255):
    ...
    if os.path.exists(directory + 'transform/'):
        ...
        # retrieve the list of filenames for images in the transformed directory.

        c = glob.glob(directory + 'transform/' + '*' + ext)

        if len(c) == len(b)*2:

            # set the flag values for return to be used in the reg_comb function
            transformed = True
            histograms = None

            # read in one of the images to get the shape and generate the arrays to be populated with the 
            ...
            # go through the list of images to extract the file_num (see original image handling for ex. )
            # and determine whether the file is a slice image or a mask and import as appropriate.
        ...
        # if the folder does not have hte correct number of files then remove the directory and all contents.
    ...
    '''
    If the transformed image folder does not exist then begin the import process.

    First, the image number of each file is extracted from the image filepath retrieved by the glob command.
    Ex. /directory/DSC0000.jpg - in this example the file_num would be retrieved as '0000' to be used in the
                                 sorting verification graph and image pairing confirmation later on.

    Next the images are imported and histogram data is generated for the sorting algorithm.
    '''
...
def registration(im1,im2):
    ...
    im1Gray = cv.GaussianBlur(im1Gray, (31, 31), 0)
    im2Gray = cv.GaussianBlur(im2Gray, (31, 31), 0)
        ...
        for i in range(len(order)):
            ...
            if i != 0:
                ...
                for j in range(10):
                    ...
                    if ind == 0:
                        cv.imshow("Image", images[order[i]])
                        cv.waitKey(1)
                    else:
                        cv.imshow("Image", images[order[trans_on[i]]] )
                        cv.waitKey(1)
                im1reg, h = registration(images[order[i]],images[order[trans_on[i]]])
                ind=0
                for j in range(10):
                    print(j)
                    ind = (ind+1) % 2
                    ...
                    if ind == 0:
                        cv.imshow("Image", im1reg)
                        cv.waitKey(1)
                    else:
                        cv.imshow("Image", images[order[trans_on[i]]] )
                        cv.waitKey(1)
                #lap_mask[order[i]] = img_warp(lap_mask[order[i]], lap_mask[order[trans_on[i]]], h)
