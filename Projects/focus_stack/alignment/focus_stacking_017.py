from stacking_funcs_009 import get_images, laplace_threshold, mask_blur, registration, pca_processing
...
filenames, images, file_nums, mask, histograms = get_images(directory, hist_thresh=hist_thresh)

print('Import:  ', time.time()-t0)

images_pca = pca_processing(images=images, filenames=filenames, file_nums=file_nums, histograms=histograms, n_comps=3, color_channels=images.shape[3], hist_thresh=hist_thresh)

print(images_pca.shape)

print(np.cov(images_pca).shape)
