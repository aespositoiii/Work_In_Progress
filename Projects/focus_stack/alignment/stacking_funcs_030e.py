...
def reg_comb(images, order, trans_on, file_nums, thresh=15, norm_blur=11, n_iter=50, exp=2, warp_mode=cv.MOTION_EUCLIDEAN, number_of_iterations=1000, termination_eps=1e-3):
    ...
    for n in range(iter) :
        ...
        cv.imshow('Lap Mask Norm', lap_mask_norm.sum(axis=0))
        cv.waitKey(1)
        ...
        for i in range(images.shape[3]):
            t6 = time.time()
            comb[:,:,i][lap_mask_norm>0] = comb[:,:,i][lap_mask_norm>0] + (images[:,:,:,i][lap_mask_norm>0] * lap_mask_norm[lap_mask_norm>0]).sum(axis=0)
            t7 = time.time()
            print(i, '   ', t7-t6, '   ', t7-t0)
        ...
        for i in range(lap_mask_float.shape[0]):
            t8 = time.time()
            lap_mask_float[i] = cv.GaussianBlur(lap_mask_float[i], (21,21), 0)
            if n == iter-2:
