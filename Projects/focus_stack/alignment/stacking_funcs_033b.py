...
def img_warp(im_obj, h):

    height, width = im_obj.shape[0:2]
    im_objReg = cv.warpPerspective(im_obj, h, (width, height))
    ...
    if transformed == False:
        ...
        for i in range(len(order)):
            ...
            if i != 0:
                ...
                images[order[i]], h = registration(images[order[i]], images[order[trans_on[i]]])
                lap_mask[order[i]] = img_warp(lap_mask[order[i]], h)
